...
variable_declaration:
    simple_type  T_ID optional_initializer
    {
      ...
      {
        case GPL::INT:
          scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, new int(42)));
          break;
        case GPL::DOUBLE:
          scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, new double(3.14159)));
          break;
        case GPL::STRING:
          scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, new std::string("Hello world")));
          break;
        default:
          assert(false);
      }
    ...
    | simple_type  T_ID T_LBRACKET T_INT_CONSTANT T_RBRACKET
    {
      ...
      {
        std::string temp = std::to_string($4);
      ...
      if($1 == GPL::INT)
      {
        int* temp_i = new int[$4];
        for(int i = 0; i < $4; i++)
        {
          temp_i[i] = 42;
        }
        scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, temp_i, $4));
      }
      else if($1 == GPL::DOUBLE)
      {
        double* temp_d = new double[$4];
        for(int i = 0; i < $4; i++)
        {
          temp_d[i] = 3.14159;
        }
        scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, temp_d, $4));
      }
      else if($1 == GPL::STRING)
      {
        std::string* temp_s = new std::string[$4];
        for(int i = 0; i < $4; i++)
        {
          temp_s[i] = "Hello world";
        }
        scopeman.add_to_current_scope(std::make_shared<Symbol>(*$2, temp_s, $4));
      }
      else
      {
        assert(false);
