...
%type <union_expression_ptr> level01;
%type <union_expression_ptr> level02;
%type <union_expression_ptr> level03;
%type <union_expression_ptr> level04;
%type <union_expression_ptr> level05;
%type <union_expression_ptr> level06;
%type <union_expression_ptr> level07;
%type <union_expression_ptr> level08;
...
level03:level04
    | level03 T_EQUAL level04 {$$=new Equal($1,$3);}
    | level03 T_NOT_EQUAL level04 {$$=new Not_Equal($1,$3);}

level04:level05
    | level04 T_LESS_EQUAL level05 {$$=new Less_Equal($1,$3);}
    | level04 T_GREATER_EQUAL  level05 {$$=new Greater_Equal($1,$3);}
    | level04 T_LESS level05 {$$=new Less($1,$3);}
    | level04 T_GREATER  level05 {$$=new Greater($1,$3);}

level05:level06
    | level05 T_PLUS level06 { $$=new Plus($1, $3);}
    | level05 T_MINUS level06
      {
        bool has_string = false;
        if($1->type() == GPL::STRING)
        {
...
level07:level08
    | T_MINUS  level07 %prec UNARY_OPS
      {
        bool not_num = false;
...
level08:primary_expression              {$$=$1;}
    | level08 T_NEAR primary_expression {$$=nullptr; /*CHANGE*/}
    | level08 T_TOUCHES primary_expression {$$=nullptr; /*CHANGE*/}
